...
app.post('/filmes/adicionar', async (req, res) => {
    try {
        if (!req.body.title) {
            ...
            const filme = await prisma.film.findFirst({
                where: {
    ...
    try {
        if (!req.body.id) {
            ...
            if (!review.filmId) {
                ...
                if (!filme) {
                    res.status(500).json({ error: 'Ocorreu um erro ao deletar a avaliação' });
                } else {
        ...
        if (!req.body.id) {
            ...
            if (!req.body.title && !req.body.rating && !req.body.content) {
                ...
                if (req.body.rating && (req.body.rating > 10 || req.body.rating < 0)) {
                    ...
                    if (!review) {
                        res.status(404).json({ error: 'Id de avaliação não existe' });
                    } else {
                        ...
                        if (req.body.rating && review.filmId) {
                            const filme = await prisma.film.findFirst({
                                where: {
                                    id: review.filmId
                                }
